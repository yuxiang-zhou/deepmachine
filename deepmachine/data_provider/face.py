...
def generate_occlusion(image, uv):
    def wrap(image, uv):
        for _ in range(int(np.random.rand() * 5)):
            ...
            image[min_y:max_y, min_x:max_x] = np.random.rand(max_y-min_y, max_x-min_x, 1) * 255
...
class Dataset:
    ...
    def get(self):
        ...
        if self.is_training:
            ...
            size = tf.reduce_min([600-begin, tf.random_uniform((), 555, 600, tf.int32)])
...
class Dataset300W(Dataset):
    def __init__(self, **kwargs):
        names = ['300w_densereg_600x600.tfrecords', 'helen_densereg_600x600.tfrecords']
        super().__init__(names, **kwargs)
