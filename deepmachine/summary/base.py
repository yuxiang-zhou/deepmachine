...
def summary_input_image(data_eps, network_eps):
    tf.summary.image('images', data_eps['inputs'], max_outputs=4)


def summary_total_loss(data_eps, network_eps):
    total_loss = tf.losses.get_total_loss()
    tf.summary.scalar('losses/total_loss', total_loss)


def summary_iuv(data_eps, network_eps):
...
def summary_landmarks(data_eps, network_eps):
